    ...
    <div class="flex justify-center">
        <div class="flex grow space-x-2">
            <Button
                    variant="destructive"
                    class="grow w-1/3"
                    disabled={!uidSet.size}
                    on:click={() => {
          console.log("将要删除：", uidSet);
    ...
    <Dialog.Root bind:open={contentDialogOpen}>
        <Dialog.Content class="sm:max-w-[425px]">
            ...
            contentDialogOpen = false;
                ...
                >
                <Button variant="destructive" on:click={confirmDeleteAccount}>是
                </Button
                >
            </Dialog.Footer>
        </Dialog.Content>
