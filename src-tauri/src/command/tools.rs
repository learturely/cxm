use cxlib::default_impl::signner::{DefaultLocationInfoGetter, DefaultQrCodeSignner};
...
pub async fn scan_image(w: u32, h: u32, image_buffer: Vec<u8>) -> Result<String, String> {
    ...
    for r in &r {
        let url = r.getText();
        // 如果符合要求的二维码。
        if !QrCodeUtils::is_enc_qrcode_url(url) {
            log::warn!("{url:?}不是有效的签到二维码！");
            continue;
        }
        log::info!("存在签到二维码。");
        // 如果不是精确截取的二维码，则不需要提示。
        return QrCodeUtils::find_qrcode_sign_enc_in_url(url)
